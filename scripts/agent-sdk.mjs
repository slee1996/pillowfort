...
export class PillowfortAgent {
  ...
  constructor({ baseURL, headed = false, maxSessions = 8, timeoutMs = 30_000, cmsURL, cmsStorageState } = {}) {
  ...
  async execute(sessionName, name, input = {}) {
    ...
    const run = async () => {
      if (paced) {
        ...
        if (waitMs) {
          try { await delay(waitMs, undefined, { signal: session.actionAbort.signal }); }
