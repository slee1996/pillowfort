...
fn wipe(mut value: Vec<u8>) {
    value.zeroize();
}

/// JSON serialization in this store frequently contains live MLS key material.
/// `serde_json::to_vec` grows a normal `Vec` by freeing predecessor allocations
/// without erasing them first, which leaves recoverable fragments in WASM linear
/// memory. This writer performs every growth explicitly and wipes the retired
/// allocation before it is released.
#[derive(Default)]
struct WipedJsonWriter {
    bytes: Vec<u8>,
}

impl WipedJsonWriter {
    fn ensure_capacity(&mut self, additional: usize) -> std::io::Result<()> {
        let required = self
            .bytes
            .len()
            .checked_add(additional)
            .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "JSON output too large"))?;
        ...
        let next_capacity = self
            ...
            .saturating_mul(2)
            .max(required);
        let mut replacement = Vec::with_capacity(next_capacity);
...
impl std::io::Write for WipedJsonWriter {
...
impl Drop for WipedJsonWriter {
    fn drop(&mut self) {
        self.bytes.zeroize();
    }
}

fn secure_json_to_vec<T: ?Sized + Serialize>(value: &T) -> Result<Vec<u8>, ::serde_json::Error> {
    let mut output = WipedJsonWriter::default();
    ::serde_json::to_writer(&mut output, value)?;
    Ok(output.into_vec())
}

// Keep the existing call sites readable while routing every serialized storage
// record through the wiped-growth writer above.
mod serde_json {
    pub use ::serde_json::{from_slice, Error};

    pub fn to_vec<T: ?Sized + serde::Serialize>(
        value: &T,
    ) -> Result<Vec<u8>, ::serde_json::Error> {
        super::secure_json_to_vec(value)
    }
}

fn replace_value(values: &mut HashMap<Vec<u8>, Vec<u8>>, key: Vec<u8>, value: Vec<u8>) {
...
fn remove_value(values: &mut HashMap<Vec<u8>, Vec<u8>>, key: &[u8]) {
    if let Some((old_key, old_value)) = values.remove_entry(key) {
        wipe(old_key);
        wipe(old_value);
    }
...
impl MemoryStorage {
    ...
    fn write<const VERSION: u16>(
        ...
        replace_value(&mut values, storage_key, value);
        Ok(())
    }
    ...
    fn append<const VERSION: u16>(
        ...
        let mut value = value;
        let mut list: Vec<Vec<u8>> = match serde_json::from_slice(list_bytes) {
            Ok(list) => list,
            Err(error) => {
                value.zeroize();
        ...
        // Serialize in a separate wiped-growth allocation so neither the new
        // value nor the predecessor can be stranded by a `Vec` reallocation.
    ...
    fn remove_item<const VERSION: u16>(
        ...
        if let Some(pos) = list.iter().position(|stored_item| stored_item == &value) {
            wipe(list.remove(pos));
        }

        // Serialize separately before erasing and replacing the predecessor.
        // The caller's comparison value and parsed list are transient copies.
        let result = serde_json::to_vec(&list);
        value.zeroize();
        for item in &mut list {
            item.zeroize();
    ...
    fn read_list<const VERSION: u16, V: Entity<VERSION>>(
        ...
        let mut value: Vec<Vec<u8>> = match values.get(&storage_key) {
    ...
    fn delete<const VERSION: u16>(
        ...
        remove_value(&mut values, &storage_key);
        storage_key.zeroize();
...
impl StorageProvider<CURRENT_VERSION> for MemoryStorage {
    ...
    >(
        ...
        replace_value(&mut values, key, value);
        Ok(())
    }
